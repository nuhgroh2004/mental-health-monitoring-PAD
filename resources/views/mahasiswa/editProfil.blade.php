    ...
    <div class="bg-white rounded-lg shadow-lg p-4 sm:p-6 md:p-8 w-full ">
        <form onsubmit="return false;" class="space-y-4 md:space-y-6">
            ...
            <div>
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nim" type="text" value="{{ old('name', $mahasiswa->NIM) }}">
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for=" prodi">
                    Prodi
                </label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="prodi" type="text" value="{{ old('name', $mahasiswa->prodi) }}">
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for="NIM">
                    Tanggal Lahir
                </label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="tanggal_lahir" type="date" value="{{ old('name', $mahasiswa->tanggal_lahir) }}">
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for="NIM">
                    Phon Number
                </label>
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nomor_hp" type="number" value="{{ old('name', $mahasiswa->nomor_hp) }}">
            </div>
            ...
            <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
                ...
                <button onclick="saveChanges()" type="button" data-url="{{ route('mahasiswa.updateProfil') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-full sm:w-1/2 flex items-center justify-center">
                    <svg class="h-5 w-5 mr-2" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
